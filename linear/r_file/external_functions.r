...
tmpl_pe<-function(betaxin1,cut_q){
  betaxin=betaxin1
  xprob[,6]=1/(1+exp(-betaxin[1]-as.matrix(xprob[,1:4])%*%(betaxin[-1]))) #phix
  
  xprob=xprob[order(xprob[,6]),] # order by phix
  xprob[,7]=cumsum(xprob[,5]) # cumsum
  for(i in 2:108){ # group
    if(xprob[i,7]<0.25|xprob[i-1,7]<cut_q[1]){
      xprob[i,8]=1
    }
    else if(xprob[i,7]<0.5|xprob[i-1,7]<cut_q[2]){
      xprob[i,8]=2
    }
    else if(xprob[i,7]<0.75|xprob[i-1,7]<cut_q[3]){
      xprob[i,8]=3
    }
    else{ xprob[i,8]=4}
...
external_infor<-function(target_beta,NN=50000,m=1000,var_ext=F){
  ...
  for(i in 1:m){
    ...
    Y <-rbinom(NN,size=1,prob=1/(1+exp(-tmp)))  
    ...
    #####Phi(X)####################################################
    glm.external <- glm(Y ~ X1+X2+X3+X4,data=Dat.external,family=binomial)
    
    betaxin_t <- as.vector(glm.external$coefficients)
    betaxin[i,]<-betaxin_t
    # betaxin_var[i,]<-as.vector(vcov(glm.external))
    if(var_ext==T){
      betaxin_var[i,]<-as.vector(summary(glm.external)$cov.scaled)
    }
  ...
  return(list(betaxin,tmplist,pe,betaxin_var,tmplist2,pe2))
  # return(mean(Y))
}
