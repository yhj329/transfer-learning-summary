...
simu_glm<-function(betaxin_total,type_ini,target_beta =c(-2.4,-0.5,0.4,0.3,0.65,0.1),random_target,source_beta,
  ...
  for(i in 1:N_r){
    ...
    # asyd
    actnn2[i,]<-as.numeric(abs(constr_cml(beta=res_cml$solution,xprobdata=xprob2,pe=pe_cml,tmpl =tmpl_cml))<1/1000000)
    
    cder<-Cderivative(betahat =res_cml$solution,xprobdata = xprob2,pe=pe_cml,tmpl =tmpl_cml)
    if(length(cder)==1){
      umat<-diag(rep(1,6))
    }else{umat<-Null(t(matrix(cder,ncol=6)))}
    
    # umat<-diag(rep(1,6))
    
    fisher_ma<-fisher_info(betahat =res_cml$solution,datax = cbind(1,X1,X2,X3,X4),datazcat = Zcat)
    b=tryCatch({list(state=1,result=umat%*%solve(t(umat)%*%fisher_ma%*%umat)%*%t(umat))},error=function(e){return(list(state=0,result=list(umat=umat,fisher_ma=fisher_ma,cder=cder)))})
    if(b$state==0){
      return(b)
    }
    ...
    middle_I<-Middle_I(datay = Y,betahat =res_cml$solution,datax =cbind(1,X1,X2,X3,X4),datazcat = Zcat )
    ...
    beta_cmle2[i,]<-res_cml$solution         # "NLOPT_LN_COBYLA"   "NLOPT_LD_MMA" "NLOPT_LD_SLSQP"
    
    
    # asyd
    actnn2[i,]<-as.numeric(abs(constr_cml(beta=res_cml$solution,xprobdata=xprob2,pe=pe_cml2,tmpl =tmpl_cml2))<1/1000000)
    
    cder<-Cderivative(betahat =res_cml$solution,xprobdata = xprob2,pe=pe_cml2,tmpl =tmpl_cml2)
    if(length(cder)==1){
      umat<-diag(rep(1,6))
    }else{umat<-Null(t(matrix(cder,ncol=6)))}
    ...
    betamy<-rootSolve::multiroot(f=function(beta)est_f(beta,xprobdata = xprob3,betaxin=betaxin_1,betaz=betazt),start = betaxin_1)$root
